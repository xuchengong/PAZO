...
class Trainer(BaseTrainer):
    def __init__(self, params):
        ...
        def compute_loss(weights, buffers, x, mask, y, pos):
            (x, mask, y, pos) = (x.unsqueeze(0), mask.unsqueeze(0), y.unsqueeze(0), pos.unsqueeze(0))
            outputs = func_model(weights, buffers, input_ids=x, attention_mask=mask, mask_pos=pos)
            loss = self.loss(outputs.logits, y)
            return loss

        self.compute_grad_and_loss = grad_and_value(compute_loss)
        self.per_sample_grad_and_loss = vmap(self.compute_grad_and_loss, in_dims=(None, None, 0, 0, 0, 0),
        ...
        for epoch in range(self.epoch_start, self.epochs):
            ...
            for i, (x, mask, y, pos) in enumerate(self.train_loader):
                ...
                for j in range(self.num_microbatches):
                    ...
                    grads, loss = self.per_sample_grad_and_loss(weights, buffers, x[lower:upper], 
                                                                mask[lower:upper], y[lower:upper], pos[lower:upper])
                    with torch.no_grad():
                        grad_tensor = []
                        for grad in grads:
                        ...
                        if epoch % 20 == 0 and i==0:
                            print('grad norm max', grad_norm.max().item(), 'min', grad_norm.min().item())
